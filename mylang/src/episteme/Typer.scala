...
object Typed {
    ...
    case class Cast(targetType: Type, exp: Expression, typ: Type)                                      extends Expression
    ...
    private def typecheckExpression(exp: Expression): Typed.Expression = {
        exp match {
            ...
            case Binary(op, exp1, exp2) => {
                ...
                if (t1 != I32() && t1 != I64()) {
                    throw EpistemicError(s"Binary operator handling requires numeric types, found: $t1")
                }
