...
class Codegener {
    ...
    private def newTempName(): String = {
        tempCounter += 1
        s"t$tempCounter"
    }
    ...
    def codegenInstruction(ins: Tac.Instruction): List[Asm.Instruction] = ins match {
        ...
        case Tac.AddPtr(ptr, index, scale, dest) => {
            val asmPtr  = codegenTacVal(ptr)
            val asmDest = codegenTacVal(dest)

            val regBase  = Asm.Register(Asm.Reg.X9)
            val regIndex = Asm.Register(Asm.Reg.X10)

            index match {
                case Tac.Constant(Const.U64Lit(idxValue)) =>
                    val totalOffset = idxValue.toLong * scale
                    List(
                      Asm.Mov(asmPtr, regBase),
                      Asm.GetAddress(Asm.PseudoReg(newTempName(), Size.Byte8), regBase),
                      Asm.Binary(Asm.BinaryOp.Add, regBase, Asm.Imm64(totalOffset), asmDest)
                    )

                case Tac.Constant(Const.I32Lit(idxValue)) =>
                    val totalOffset = idxValue.toLong * scale
                    List(
                      Asm.Mov(asmPtr, regBase),
                      Asm.Binary(Asm.BinaryOp.Add, regBase, Asm.Imm64(totalOffset), asmDest)
                ...
                case dynamicIndex =>
                    val asmIndex = codegenTacVal(dynamicIndex)

                    if (scale == 1 || scale == 2 || scale == 4 || scale == 8) {
                        List(
                          Asm.Mov(asmPtr, regBase),
                          Asm.Mov(asmIndex, regIndex),
                          Asm.GetAddress(Asm.Indexed(Asm.Reg.X9, Asm.Reg.X10, scale), asmDest)
                        )
                    } else {
                        List(
                          Asm.Mov(asmPtr, regBase),
                          Asm.Mov(asmIndex, regIndex),
                          Asm.Binary(Asm.BinaryOp.Mult, regIndex, Asm.Imm32(scale), regIndex),
                          Asm.GetAddress(Asm.Indexed(Asm.Reg.X9, Asm.Reg.X10, 1), asmDest)
                        )
                    }
            }
        }

        case Tac.CopyToOffset(src, dest, offset) => {
            val asmSrc  = codegenTacVal(src)
            val asmDest = codegenTacVal(dest)
            val ptrTemp = Asm.PseudoReg(newTempName(), Size.Byte8)

            List(
              Asm.GetAddress(asmDest, ptrTemp),
              Asm.Binary(Asm.BinaryOp.Add, ptrTemp, Asm.Imm32(offset), ptrTemp),
              Asm.StoreIndirect(asmSrc, ptrTemp)
            )
        }
        ...
        case Tac.Binary(Tac.BinaryOp.Remainder, src1, src2, dest) => {
            ...
            val pseudoQuotient = d match {
                ...
                case _                        => Asm.PseudoReg(newTempName(), Size.fromTacType(t))
            }
